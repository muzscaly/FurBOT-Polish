...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("Wyłączone użycie `{}`".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Ta komenda nie może być wyłączona")

        else:
            update.effective_message.reply_text("Co mam wyłączyć?")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("Włączone użycie `{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Czy to jest wogóle wyłączone?")

        else:
            update.effective_message.reply_text("Co mam włączyć?")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("Następujące komendy są możliwe do przełączania:\n{}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text("Żadnych komend nie można wyłączyć.")
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Żadna komenda jest wyłączona!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Następujące komendy są obecnie wyłączone:\n{}".format(result)
    ...
    def __stats__():
        return "{} wyłączonych rzeczy pośród {} czatów.".format(sql.num_disabled(), sql.num_chats())
    ...
    __mod_name__ = "Wyłączanie komend"
